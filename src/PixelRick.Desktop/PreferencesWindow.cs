...
internal sealed class PreferencesWindow : Window
{
    public PreferencesWindow(ConfigurationStore store, IStartupService? startup, Action applied)
    {
        ...
        Background = new SolidColorBrush(Color.Parse("#17202D"));
        ...
        var c = store.Value;
        Slider Slider(string name, double min, double max, double value, double tick) { var label = new TextBlock(); var s = new Slider { Minimum = min, Maximum = max, Value = value, TickFrequency = tick, IsSnapToTickEnabled = true, Height = 28, MinHeight = 28 }; void Update() => label.Text = $"{name}: {s.Value:0.##}"; s.PropertyChanged += (_, e) => { if (e.Property == Avalonia.Controls.Slider.ValueProperty) Update(); }; Update(); panel.Children.Add(label); panel.Children.Add(s); return s; }
        CheckBox Check(string name, bool value) { var item = new CheckBox { Content = name, IsChecked = value, Height = 28, MinHeight = 28 }; panel.Children.Add(item); return item; }
